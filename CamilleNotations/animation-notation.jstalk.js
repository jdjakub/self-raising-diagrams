  ...
  ['Root']: (self) => {
    ...
    seqs = seqs.map(([triples, nodes, seqName]) => {
      ...
      for (const [from,event,to] of triples) {
        ...
        if (!fromObj) fromObj = objs[⟦from id⟧] = { cmds: [], byEvent: {} };
        if (!toObj)     toObj = objs[⟦to id⟧]   = { cmds: [], byEvent: {} };
        fromObj.byEvent[event] = ⟦to id⟧;
      }
      ...
      for (const node of nodes) {
        ...
        const cmdsString = ⟦byId(nodeId) string⟧;
...
vtables.AnimationController = {
  ['doEvent:']: (self, event) => {
    ...
    if (isWait) ⟦self waitComplete: [event,timerId]⟧;
    self.timerId++;
  },
  ['execute:on:']: (self, [op,...args], elt) => {
    if (op === 'opacity') {
      ...
      attrs(elt, {transform: 'scale('+args[0]+')', transform_origin: ⟦elt center⟧.join(' ') });
    } else if (op === 'text') {
      // SMELL DOM munging
      ...
      ⟦self waitFor: secondsToWait withPrefix: args[1]⟧;
    } else if (op === 'anim') {
      ...
      const cmdToKeyVal = cmd => {
        if (cmd[0] === 'stroke') return cmd;
          ...
          attr(elt, 'transform-origin', ⟦elt center⟧.join(' '));
      ...
      ⟦self execute: ['wait', duration, 'anim'] on: elt⟧; // End with a wait
    }
  },
