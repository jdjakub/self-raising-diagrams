vtables.BoardGameNotation = {
  ...
  ['TransformSpec']: (self) => {
    const rules = send(self, 'many1:', () => send(self, 'Before:', 'ZonePat', 'After:', 'SinglePiece'))
                  .map(nt => nt.inner); // We don't expect names
    return rules.map(({before, after}) => {
      const beforeZone = send(self, 'identifyZone:', before);
      // SMELL duped from BoardPat
      const contained = send(before, 'interior').children;
  ...
  ['BoardState']: (self) => {
    let pieces = send(self, 'apply:', 'BoardPat');
