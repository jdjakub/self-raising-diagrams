...
// JSTalk sugar ⟦ super doSomething: blah1 with: blah2 ⟧
...
vtables.domNode = {
  ...
  ['signedDistanceTo:']: (self, other) => {
    ...
    return ⟦self encloses: other⟧ ? -dist : dist;
  },
  ...
  ['reroot']: (self) => { // Given: containedIn
    ...
    let parentRoot = ⟦soonToBeParent localRoot⟧;
  ...
  ['interior']: (self) => ⟦⟦self localRoot⟧ interior⟧,
};
...
vtables.byTag['path'] = {
  ...
  ['outwardTangentAt:']: (self, frac /* 0 to 1 */) => {
    const tangent = ⟦self tangentAtFrac: frac⟧;
  ...
  ['specialize']: (self) => {
    ...
    if (!⟦self isCurved⟧) { // => Polygon | Polyline
      const polygons = polysFromPath(⟦self commands⟧);
  ...
  ['endpointAt:']: (self, i/* 0 or 1 */) => {
    if (⟦self isClosed⟧) throw [self, ' is closed, no endpoints'];
    if (i !== 0 && i !== 1) throw [i, ' must be 0 or 1'];
    return ⟦self pointAtFrac: i⟧;
  },
  ...
  ['bakeTransform']: (self) => {
    ...
    if (!t) return false;
    const m = t.matrix;
    const d = ⟦self commands⟧.map(([cmd, ...pts]) => {
...
vtables.byTag['g'] = {
  ...
  ['containsPt:']: (self, pt) => {
    for (let c of [...self.children])
      if (⟦c containsPt: pt⟧) return true;
    return false;
  },
  ['encloses:']: needsSuper((supr, self, other) =>
    self.classList.contains('text-wrapper') ? ⟦super encloses: other⟧
    : self.firstChild && ⟦self.firstChild encloses: other⟧ // Assumes wrapped shape
  ),
  ['specialize']: (self) => {
    let anySpecialized = false;
    for (let c of [...self.children])
      if (⟦c specialize⟧) anySpecialized = true;
  ...
  ['lineSegments']: needsSuper((supr, self) => {
    if (self.matches('.connector-wrapper'))
      return ⟦self.querySelector('.connector-shaft') lineSegments⟧;
    return ⟦super lineSegments⟧;
  }),
  ...
  ['isIncidentOn:']: (self, other) => {
    ...
    const closestToOrigin = ⟦other closestPtToPt: originPt⟧;
    const closestToTarget = ⟦other closestPtToPt: targetPt⟧;
...
similarShapes = function(sh1, sh2) {
  ...
  if (⟦sh1 isCurved⟧) {
    ...
    const [vs1, vs2] = [⟦sh1 vertices⟧, ⟦sh2 vertices⟧];
...
vtables.byTag['path']['target'] = (self) => {
  ...
  let target = nilElem;
  const targetPt = ⟦self targetPt⟧;
  const originPt = ⟦self originPt⟧;
...
vtables.byTag['path']['isDirected'] = (self) => ⟦self targetPt⟧ !== null;

vtables.byTag['path']['originPt'] = (self) =>
  match(vtables.MathchaArrow, ⟦self localRoot⟧, 'originPt');

vtables.byTag['path']['targetPt'] = (self) => 
  match(vtables.MathchaArrow, ⟦self localRoot⟧, 'targetPt');
...
function init() {
  ...
  ⟦vocab init⟧; // Normalize shapes/connectors/text exported under different editors' encodings
  svg_parent.dataset.normalized = true;

  // First, gather all exported shapes and text.
  let elems = ⟦vocab seedElements⟧;
  ...
  all('g.connector-wrapper').forEach(g => ⟦g id⟧); // Force an ID so containments work
  ...
  elems.forEach(el => ⟦el findTightestContainerIn:
