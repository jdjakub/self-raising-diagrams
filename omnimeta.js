...
vtables.RegionSubstrate = {
  ...
  ['claimMatching:']: (self, designator) => {
    ...
    const sel = (selRule && hasRule(self, selRule)) ? send(self, 'apply:', selRule) : null;
    return send(self,
      'claimFirst:', (elem) => {
...
vtables.NotationGrammar = {
  ...
  ['nameTextNear:']: (self, thing) =>
    send(self, 'claimFirst:', (elem) => {
          ...
          'quickTest:', hasRule(self, 'LabelSel')
                          ? (e => e.matches(send(self, 'LabelSel')))
                          : null),
  ...
  ['claimAll:near:within:']: (self, rule, anchor, eps) => {
    ...
    const sel = hasRule(self, selRule) ? send(self, 'apply:', selRule) : null;
    return send(self, 'many:', () => send(self, 'claimFirst:', (elem) => {
      if (send(anchor, 'signedDistanceTo:', elem) > eps) return null;
      const saved = self.cursor;
      self.cursor = { ...saved, dict: elem };
      try { const v = send(self, 'applyRule:', rule); self.cursor = saved; return { value: v }; }
  ...
  ['Box:']: (self, inner) => {
    const box = send(self, 'claimMatching:', 'BoxShape');
    return send(self, 'lend:', self.vtable, 'input:', send(box, 'interior'), 'rule:', inner);
  },
...
vtables['GraphObject'] = {
  ...
  ['connectionsOf:']: (self, edge) => {
    ...
    edge._connections = send(edge, 'endpoints').map(pt => send(self, 'nodeAt:', pt));
...
vtables.BoxGraphOM = {
  ...
  ['BoxGraph']: (self) => send(self, 'GraphWithVertices:', (self) => send(self, 'Named:', 'Box'),
...
vtables.PowerpointArrowPath = {
  ...
  ['arrow']: (self) => {
    ...
    send(self, 'many:', () => {
      send(self, 'not:', () => { 
        send(self, 'opt:', () => send(self, 'apply:', 'cmd:', 'with:', ['L']));
...
vtables.MathchaVocab = {
  ...
  ['parseTextElt:']: (self, g) => send(g, 'parseAsParagraph'),   // Mathcha: g/g/g/text
